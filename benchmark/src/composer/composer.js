    ...
    installSmartContract() {
        ...
        return busnets.reduce((promiseChain, busnet) => {
            return promiseChain.then(() => {
                return composer_utils.runtimeInstall(busnet, null, 'PeerAdmin' + busnet.org + '@hlfv1');
            });
        }, Promise.resolve())
            .then((result) => {
            // network start on single peer using organization card
                return chaincodes.reduce((promiseChain, busnet) => {
                    return promiseChain.then(() => {
                        return composer_utils.networkStart(busnet, 'PeerAdmin' + busnet.orgs[0] + '@hlfv1', busnet.loglevel);
                    });
                }, Promise.resolve());
            })
            .then(() => {
