...
describe('Sample', () => {
    ...
    beforeEach(async () => {
        ...
        const Admin1 = factory.newTransaction('transaction.manager', 'CreateUser');
        Admin1.uid = 'Admin1';
        Admin1.name = 'Admin1';
        Admin1.identify = 'Admin1';
        Admin1.manager = true;
        await businessNetworkConnection.submitTransaction(Admin1);
        let identity = await businessNetworkConnection.issueIdentity('manager.User' + '#Admin1', 'Admin1',{issuer:true});
        await importCardForIdentity('Admin1', identity);
        await useIdentity('Admin1');
        const User1 = factory.newTransaction('transaction.manager', 'CreateUser');
        User1.uid = 'User1';
        User1.name = 'User1';
        User1.identify = 'User1';
        User1.manager = false;
        await businessNetworkConnection.submitTransaction(User1);
        identity = await businessNetworkConnection.issueIdentity('manager.User' + '#User1', 'User1');
        await importCardForIdentity('User1', identity);
        await useIdentity('User1');
    ...
    it('Alice can read all of the assets', async () => {
        ...
        await useIdentity('User1');
        const CreateFileEncrypted = factory.newTransaction('transaction.file', 'CreateFileEncrypted');
        const AccessInfo = factory.newConcept('file','AccessInfo');
        const Crypto = factory.newConcept('file','Crypto');
        AccessInfo.user = factory.newRelationship('manager','User','User1');
        AccessInfo.crypto_list = [Crypto];
        Crypto.issuer = factory.newRelationship('manager','User','User1');
        const ping = await businessNetworkConnection.ping(); 
        Crypto.identity = factory.newRelationship('org.hyperledger.composer.system','Identity',ping.identity.split('#')[1]);
        Crypto.encrypted_key = 'sadas';
        Crypto.public_key = 'asdasdasdas';
        CreateFileEncrypted.uid = '/User1';
        CreateFileEncrypted.access_info_list = [AccessInfo];
        CreateFileEncrypted.control_info = factory.newConcept('file','ControlInfo');
        CreateFileEncrypted.control_info.required_list = [factory.newRelationship('manager','User','User1')];
        CreateFileEncrypted.control_info.optional_list = [];
        CreateFileEncrypted.control_info.thresh_hold = 0;
        CreateFileEncrypted.meta_data = '';
        CreateFileEncrypted.checksum = 's12321';
        await businessNetworkConnection.submitTransaction(CreateFileEncrypted);
        const assetRegistry = await businessNetworkConnection.getAssetRegistry('file.FileEncrypted');
